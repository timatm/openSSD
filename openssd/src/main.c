...
int main()
{
    ...
    pr_info("NMC Buffers Range: %X ~ %X", NMC_BUFFERS_START_ADDR, NMC_BUFFERS_END_ADDR);
...
#define MB (1024 * 1024)
    for (u = 0; u < 4096; u++)
    {
        ...
        else if (NMC_BUFFERS_START_MB <= u && u <= NMC_BUFFERS_END_MB)
        {
            pr_info("TLB attribute: set uncached & nonbuffered at %u MB for NMC", u);
            Xil_SetTlbAttributes(u * MB, 0xC12); // uncached & nonbuffered
        }
        else if (u < 0x400)
            Xil_SetTlbAttributes(u * MB, 0xC1E); // cached & buffered
